...
def get_od_model():
    global modelod
    modelod = load_model('Models/od_model.hdf5',custom_objects={'dice_coef':dice_coef,'iu':iu,'iouLoss':iouLoss,'acc':acc,'IOU':IOU})
    print('*OD Model loaded!!')
def get_oc_model():
    global modeloc
    modeloc = load_model('Models/oc_model.hdf5',custom_objects={'dice_coef':dice_coef,'iu':iu,'iouLoss':iouLoss,'acc':acc,'IOU':IOU})
...
def classifier(status=None):
	return render_template("classifier.html", var=status)
...
@app.route("/predict_classifier")
def predict_classifier():
    temp = pathlib.PosixPath
    pathlib.PosixPath = pathlib.WindowsPath
    model = load_learner(classifier_path)
    img = imread("static/Images/input.jpg")
    out= model.predict(img)[0] 

    return redirect(url_for('classifier',status=out))
print(modelod.summary())
